...
export const signUp = genericFunc(async(req,res,next) => {
    ...
    const result = await databasePool.query("SELECT `username`,`email`,`phone` FROM `users` WHERE email = ? OR phone = ? OR username = ?",[email,phone,username])

    const user = result[0] as IUser[]

    if(user.length > 1){
        const message = (`${username}`.match(user[0].username)) ? errorCodes.USERNAME_ALREADY_USING :
        (`${email}`.match(user[0].email)) ? errorCodes.EMAIL_ALREADY_USING : 
        (`${phone}`.match(user[0].phone)) ? errorCodes.PHONE_ALREADY_EXISTS : "Something went wrong"
...
export const resetPasssword = genericFunc(async (req,res,next) => {
    ...
    const query = await databasePool.query("SELECT `email` FROM `users` WHERE email = ?",[email])
